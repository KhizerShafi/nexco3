...
function Services() {
  ...
  {services.map((service, index) => (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`grid md:grid-cols-2 gap-8 items-center rounded-2xl border ${service.borderColor} p-8 bg-gradient-to-r ${service.color}`}
    >
      {/* Left: Text */}
      <div>
        <div className="flex items-center gap-3 mb-4">
          <div className="p-3 rounded-lg text-white bg-white/10 border border-white/20">
            {service.icon}
          </div>
          <h3 className="text-3xl font-bold text-orange-400">{service.title}</h3>
        </div>
